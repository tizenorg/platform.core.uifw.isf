                                ...
                                Ecore_IMF_Event      *event EINA_UNUSED)
{
    ...
    if (type == ECORE_IMF_EVENT_MOUSE_UP) {
        ...
        for (i = 0; i < 30000999; i++){
            //ecore_wl_sync();
            wl_display_roundtrip(ecore_wl_display_get());
            //LOGD ("i = %d,last = %d: require = %d", i, imcontext->last_key_event_filter.serial, serial);
        ...
        LOGD ("i : %d,serial (last,require) : (%d,%d)", i, imcontext->last_key_event_filter.serial, serial);
