...
static void launch_helper (const char *name, const char *appid, const char *config, const char *display)
{
    int pid;
    ...
    for (size_t i = 0; i < _ime_info.size (); ++i) {
        if (_ime_info[i].appid == uuid && _ime_info[i].module_name.length ()) {
            ...
            if (scim_helper_path != String (SCIM_HELPER_LAUNCHER_PROGRAM)) {
                ...
                launch_helper (_ime_info[i].module_name.c_str (), uuid.c_str (), config.c_str (), display.c_str ());
            }
