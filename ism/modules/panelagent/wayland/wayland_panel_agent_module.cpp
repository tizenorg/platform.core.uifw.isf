...
isf_wsc_context_send_default_text (WSCContextISF* wsc_ctx, const char *text, int cursor)
{
    ...
    if (context_scim && context_scim->impl) {
        ...
        if (strcmp (conv_text, text) != 0 || context_scim->impl->cursor_pos != cursor) {
            ...
            //strncpy(context_scim->impl->surrounding_text, text, sizeof(context_scim->impl->surrounding_text));
            context_scim->impl->cursor_pos = cursor;

            String default_text = utf8_wcstombs (context_scim->impl->surrounding_text);
            g_info_manager->remoteinput_callback_default_text (String (text), context_scim->impl->cursor_pos);
            free(text);
        }
        free (conv_text);
    }
